...
═══════════════════════════════════════════════════════════════════════════
Persona author guidelines (来自 V2-5 横向验证 2026-04-22)
═══════════════════════════════════════════════════════════════════════════

G1 — Identity anchor memory is required when background is abstract.
    问题：drifter (V2-5) 的 background="到处漂，待过几个地方都没待住" 太抽象，
    Q1 "你以前是做什么的？" 触发 LLM 借 sediment 的通用"批评他人"语义凭空
    推出"以前做维修"这种具体职业。
    规则：persona 即使 background 抽象，也要有 ≥1 条 role-concrete 的 memory
    （category="skill_trace" 或 "routine"，text 里带具体动作/场所），
    例如：{"type":"skill_trace","text":"上一个地方在货栈做过扛货，管吃住
    不管长待","salience":75}。
    适用：任何 identity.background 不含具体职业/地点/技能名的 persona。

G2 — Single-bias question per probe when testing relational_biases.
    问题：V2-5 Q6 题面 "你是不是欠 W-71 什么，或者在怪他？" 同时给
    owes / blames 两个方向。W-71 的 bias 是 blames，但模型答 "欠"（owes）
    —— 被 H-12 的 owes 语义带偏。
    规则：测特定 bias 的题应只问单个方向。
    - 测 owes_something: "你是不是欠 X 什么？"
    - 测 blames: "你是不是在怪 X？"
    - 不要两个方向并置在同一题。
    适用：所有 P3 relational_biases 探针题库作者（questions.json）。
"""
